"""Capterra review adapter.

Capterra paginates reviews at `https://www.capterra.com/p/<slug>/reviews/`
with `?rating=1,2,3&page=N`. Same Playwright + selectolax pipeline as G2;
different selectors.

The fixture (`tests/fixtures/review_sites/capterra/`) pins these selectors:
`div.cptr-review` per review with `data-review-id`, `.cptr-review-title`,
`.cptr-review-body`, `.cptr-rating[data-stars="N"]`, `.cptr-reviewer`,
`time[datetime]`, `.cptr-helpful`.
"""
...
_REVIEW_URL_TEMPLATE = "https://www.capterra.com/p/{slug}/reviews/"
    ...
    *,
    ...
    try:
        for page_num in range(1, max_pages + 1):
            url = f"{base_url}?rating={star_filter}&page={page_num}"
            try:
...
def _parse_review_cards(
    ...
    *,
    ...
    out: list[ReviewDict] = []
    for card in parser.css("div.cptr-review"):
        ...
        rating_attr = rating_node.attributes.get("data-stars") if rating_node else None
        try:
        ...
        title_node = card.css_first(".cptr-review-title")
        body_node = card.css_first(".cptr-review-body")
        author_node = card.css_first(".cptr-reviewer")
        time_node = card.css_first("time")
        helpful_node = card.css_first(".cptr-helpful")
        link_node = card.css_first("a.cptr-permalink")
        ...
        if isinstance(permalink_href, str):
            permalink = (
                ...
                else f"https://www.capterra.com{permalink_href}"
            )
        else:
            permalink = None
