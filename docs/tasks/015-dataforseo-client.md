...
**Goal:** A thin async wrapper around DataForSEO's REST API with a hard monthly cost cap.
...
- Budget guard: before each call, sum `api_usage.est_cost_usd` for the current calendar month; if ≥ `APFUN_DFS_MONTHLY_CAP_USD` (default 25.0), raise `BudgetExceeded`. Pipeline callers must handle this by marking the candidate `status=budget_blocked` and writing a `scheduler_runs` note.
