...
- 2026-05-25: **Schema read at app *startup* must degrade gracefully, or a code-before-migration deploy takes the whole service down — not just the routes that read it.** The `--reload` deploy model boots the new code the instant `git pull` lands, *before* the operator runs `make migrate`. Request handlers that query a not-yet-created column/table just 500 per-request (the buildability deploy: `/inbox` 500'd, rest fine). But the FastAPI **lifespan** reading new schema (014-fix-2 read `runtime_state` to re-apply a persisted scheduler pause) raises during startup → uvicorn's worker exits → and because the migration touched the DB, not a watched `.py` file, `--reload` sits idle (no retry) → `/inbox` + `/ops` + everything is down until you migrate AND touch a watched file to wake the reloader. Two-part fix: (1) wrap any startup-time read of operator-/migration-managed schema in `try/except` that logs and degrades to a safe default (`apfun/main.py` lifespan now treats a failed `runtime_state` read as "not paused"); (2) operationally, a migration whose columns/tables are read at startup should run *with or before* the code, and after `make migrate` you may need to `touch apfun/main.py` to re-trigger a reloader that idled on the crashed boot. Generalizable: startup paths are fail-closed by default (a raised exception = no service); make them fail-open for anything that depends on a separately-sequenced migration. Tuition: shipping 014-fix-2.
