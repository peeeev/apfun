...
- **Verify external constants inline.** Any numeric or string constant sourced externally — pricing, API rate limits, model identifiers, third-party-published timeouts, TTLs, UA-string requirements — carries a `# verified YYYY-MM-DD <source-url-or-doc-path>` comment in the line(s) immediately above. Constants without this annotation are treated as unverified during code review. When you touch an unverified constant during a task, verify it as part of that task — don't defer. The convention IS the audit; we deliberately don't keep a standalone audit script (they decay).
