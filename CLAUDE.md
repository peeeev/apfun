...
- **Verify external constants inline.** Any numeric or string constant whose authoritative source lives OUTSIDE this repository — pricing pages, API rate-limit specs, model identifiers, third-party-published timeouts/TTLs, UA-string requirements — carries a `# verified YYYY-MM-DD <source-url-or-doc-path>` comment immediately above. Internal project decisions (defaults you chose, enum values, computed bounds, host/port settings) don't need annotation — their authority is the codebase. When in doubt, ask: *"if a teammate questioned this number, would I cite an external URL or a project decision?"* External constants without this annotation are treated as unverified during code review; when you touch one, verify it as part of that task — don't defer. The convention IS the audit; no standalone audit scripts (they decay).
...
- 2026-05-18: External constants require a `# verified YYYY-MM-DD <source>` annotation inline; see "Verify external constants inline" in Project conventions. The convention is the audit — no standalone `verify_*.py` scripts (they decay).
